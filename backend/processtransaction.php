...
if(isset($_POST['confirm'])){
		...
		if(!is_numeric($amount)){
			$message = "What you entered is not a number.";
				$_SESSION['error']= $message;
		       header("location:confirminvestment.php");
			   exit(0);
...
/* Process data when all fields are correct */
         ...
         $sql = "SELECT * FROM tbl_investments WHERE username='$buyer_username' AND completed='yes'";
		 ...
		 // The user has PH before
	      ...
	      $sql_ship = "UPDATE tbl_investments SET amount_invested='$amount', date_invested='$deposit_date', 
            ...
            $bonus = 500;
		    $sql = "SELECT referer FROM tizeti_db_user WHERE username = '$buyer_username'";
            ...
            if ($stmt) {
                ...
                mysqli_query($cxn, "UPDATE ref_bonus SET bonus=bonus + '$bonus' WHERE username='$referer'");
            }else{
                mysqli_query($cxn, "INSERT INTO ref_bonus(username, bonus) VALUES('$referer', '$bonus')");
            }           
               // Insert the new user into the database
	          $sql_ship = "UPDATE tbl_investments SET amount_invested='$amount', date_invested='$deposit_date', 
		                            expected_amount='$expected_amount', date_expected='$exp_date', confirmed='yes', completed='no' WHERE transid='$transid'";
