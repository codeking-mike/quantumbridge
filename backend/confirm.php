 ...
 if(isset($_GET['md'])){
     ...
     $sql = "SELECT * FROM mergetable WHERE mergeid='$mergeid'";
          ...
          $pquery = "SELECT * FROM tbl_investments WHERE transid='$payer_id'";
            ...
            if($merged =='yes'){
                $deposit_date = date('Y-m-d H:i:s');
		        $exp_date = date('Y-m-d H:i:s', strtotime($deposit_date . '+5 days'));
		        $expected_amount = $amount_inv2 + ($amount_inv2 * 0.5);
		        $ref_bonus = (0.05 * $amount_inv2);
		 
           
            		 //check if user has PH before
		    $check_query = "SELECT * FROM tbl_investments WHERE payer='$payer_username' AND fulfilled='yes'";
            ...
            if($merged =='yes'){
                ...
                mysqli_query($cxn, "UPDATE tbl_investments SET fulfilled='yes' WHERE transid='$transid'");
            }
            }
